map <buffer> <S-e> :w<CR>:!/usr/bin/php % <CR>
map <buffer> <S-c> :w<CR>:!/usr/bin/php -l % <CR>
...
function! OpenPhpFunction (keyword)
  ...
  " exe 'call search("Description")' " exe 'norm 8kdgg'
...
inoremap <C-p> <C-O>:call OpenPhpFunction('<c-r><c-w>')<CR><C-O>:wincmd p<CR>:syntax on<CR>
nnoremap <C-p> :call OpenPhpFunction('<c-r><c-w>')<CR>:wincmd p<CR>:syntax on<CR>
vnoremap <C-p> :call OpenPhpFunction('<c-r><c-w>')<CR>:wincmd p<CR>:syntax on<CR>
...
if isdirectory('tests')
  autocmd BufWritePost * !phpunit tests/
endif
